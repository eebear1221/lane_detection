...
//	#pragma omp parallel for
	for (int i=anchor_point.y-search_unit;i<=anchor_point.y+search_unit;i++)
	{
		for (int j=anchor_point.x-search_unit;j<=anchor_point.x+search_unit;j++)
		{
			...
			if (mat_get_maxvalue.ptr<Vec3f>(i)[j][0]>max_value)
			{
				max_value = mat_get_maxvalue.ptr<Vec3f>(i)[j][0];
				max_value_point.x = j;
				max_value_point.y = i;
			}
		}
...
int lanedetector::vote_lines(Mat& mat_vote_grid, Vec4f& k_b_mean, vector<Vec2f>& line_fitting_info)
{
	...
	mat_vote_grid.create(m_slope_angle_range/m_slope_angle_scale, m_image_width/m_b_scale, CV_32FC3);
	...
	for (int j = 0; j < MAX_HISTORY_FRAM; j++)
	{
		...
		for (int i=0;i<cur_lineinfo.size();i++)
		{
			...
			mat_vote_grid.ptr<Vec3f>(angle_index)[b_index][0] += 1;
			mat_vote_grid.ptr<Vec3f>(angle_index)[b_index][1] += cur_lineinfo[i][0];
			mat_vote_grid.ptr<Vec3f>(angle_index)[b_index][2] += cur_lineinfo[i][1];
		}
	}
#else
...
//	#pragma omp parallel for
	for (int i=0;i<mat_vote.rows;i++)
	{
		...
		{
			if (mat_vote.ptr<Vec3f>(i)[j][0]>max_value)
			{
				max_value = mat_vote.ptr<Vec3f>(i)[j][0];
				max_pos.x = j;
				max_pos.y = i;
			}
		}
	...
	if (base_line.x != 0)
	{
		for (int i=0;i<mat_vote.rows;i++)
		{
			for (int j=0;j<mat_vote.cols;j++)
			{
				if (mat_vote.ptr<Vec3f>(i)[j][0] == max_value)
				{
					tmp_k = mat_vote.ptr<Vec3f>(i)[j][1]/ mat_vote.ptr<Vec3f>(i)[j][0];
	...
	if (mat_vote.at<Vec3f>(max_pos.y, max_pos.x)[0] > 0)
	{
		base_line.x = mat_vote.ptr<Vec3f>(max_pos.y)[max_pos.x][1]/ mat_vote.ptr<Vec3f>(max_pos.y)[max_pos.x][0];
		base_line.y = mat_vote.ptr<Vec3f>(max_pos.y)[max_pos.x][2]/ mat_vote.ptr<Vec3f>(max_pos.y)[max_pos.x][0];
	}
	else{
		base_line.x = 0.0;
		base_line.y = 0.0;
...
//		#pragma omp parallel for
	...
	if (max_vote_pos.x != 0 && max_vote_pos.y != 0 && max_vote_value != 0)
	{
			...
			)
		{
			...
			{
				Vec3f line_data =  mat_vote.ptr<Vec3f>(nearby_max_point.y)[nearby_max_point.x];
				mat_vote.setTo(Scalar::all(0));
				//mat_vote.at<Vec3f>(max_pos.y, max_pos.x) = Vec3f(0, 0, 0);
				mat_vote.ptr<Vec3f>(nearby_max_point.y)[nearby_max_point.x] = Vec3f(nearby_max_value, line_data[1], line_data[2]);
	...
	{
		base_line.x = mat_vote.ptr<Vec3f>(max_vote_pos.y)[max_vote_pos.x][1]/ mat_vote.ptr<Vec3f>(max_vote_pos.y)[max_pos.x][0];
		base_line.y = mat_vote.ptr<Vec3f>(max_vote_pos.y)[max_vote_pos.x][2]/ mat_vote.ptr<Vec3f>(max_vote_pos.y)[max_pos.x][0];
...
int lanedetector::run_lane_detection(const Mat& img, const Mat& vd_img)
{
	...
	if (atan(m_k_b_mean[0]) < -m_depart_angle_thres && atan(m_k_b_mean[2]) < m_depart_angle_thres && atan(m_k_b_mean[2]) != 0 && m_depart_interval ==0/*&& m_stable_count > LANE_STABLE_FRAMES*/)     //left lane departure
	{
	...
	if (atan(m_k_b_mean[2]) > m_depart_angle_thres && atan(m_k_b_mean[0]) > -m_depart_angle_thres && atan(m_k_b_mean[0]) != 0 && m_depart_interval ==0/*&& m_stable_count > LANE_STABLE_FRAMES*/)     //right lane departure
	{
